...
たとえば、意味の曖昧性（例:「娘がいる」はmy daughter なのか、a girl なのか）や、文脈依存性（例:「それ、やった？」は「もう終わった？」なのか「やったことある？」なのか）などは、数理的には解決が難しい。
...
また、調整文法であるならば、文脈依存性を考慮し、「それ、やった？」という曖昧性の問題は、「もう終わった？」「やったことある？」のように語句を追加することで解決される。
    \else
Language is realized through physical resources (cognitive processes of the brain, speech, symbol manipulation, etc.), making it possible to describe it mathematically.
However, language is not just a physical system, and the components used to model language differ in nature from the physical quantities used in physics, making mathematical descriptions difficult.
...
For example, semantic ambiguity (e.g., ``Musume ga iru.'' can mean either ``my daughter'' or ``a girl'') and context dependency (e.g., ``Sore, yatta?'' can mean either ``Have you finished it?'' or ``Have you ever done it?'') are difficult to resolve mathematically.
...
Also, if it falls under adjustive grammar, considering context dependency, the ambiguity in ``Sore, yatta?'' can be resolved by adding words during the adjustment process, for example, by specifying whether it means ``Have you finished it?'' or ``Have you ever done it?''
    \fi
